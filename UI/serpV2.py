...
with claim:
    company_claim = st.text_input('Company claim (optional):', help= 'short description/slogan or claim of your company', key='com_cla')
        ...
        if len(input_df.Attributes) > 10:
            ...
            if st.button('SERP my file!') and user_file:
                if company_name != "":
                    ...
                    with st.spinner("Creating your SERPed CSV file..."):
                        output_df = gen_serp_mass(user_input_vals)
                        output_csv = output_df.to_csv()
                        with st.expander('Preview of your SERPed CSV file'):
