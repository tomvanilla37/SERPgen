...
def key_to_text(                             # default value; explaination/observations
        key_list,                            # any keywords to generate sentence from
        max_length: int = 512,               # 512; max token length of prediction; 1024 not working
        num_return_sequences: int = 2,       # 1; number of predictions to be returned; 5 max
        num_beams: int = 5,                  # 2; picks N best sequences so far and considers the probabilities of combination of all preceding words along with word in current position
        top_k: int = 60,                     # 50; lower number = shorter sentence
        top_p: float = 0.90,                 # 0.95; towards 0 = less creative/more accurate, towards 1 = more creative/random
        do_sample: bool = True,              # True;
        repetition_penalty: float = 3.5,     # 2.5; avoid sentences that repeat themselves
        length_penalty: float = 1.0,         # 1.0;
        early_stopping: bool = True,         # True;    False makes sentence longer (makes up stuff)
...
def gen_serp_mass(user_input_vals):
    ...
    for index, row in mass_serp_df.iterrows():
        ...
        print(serp_output)
        mass_serp_df.loc[index, ("Output (SERP text)")] = serp_output
